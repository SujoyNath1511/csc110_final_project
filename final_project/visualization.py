...
def get_frames(x_value: list, y_value: List[list], indexes: List[int]) -> list:
    ...
    points from the first one to the (k - 1)-th one, where k is between 1 and len(x_value), inclusive.
...
def get_layout(frames: list, split_point: int, xrange: List[List[float]], yrange: List[List[float]],
               visible: Dict[str, List[bool]]) -> go.Layout():
    ...
    (i) Three options in the dropdown menu on the top:
        - 'Regression Models': to show our aggregated temperature-sea level data and our regression lines.
    ...
    (ii) Three buttons on the right:
        ...
        - 'Predict': can be clicked to show our predicted data for future time, i.e. extended regression lines.
